...
def viewappointments(request):
	...
	g = request.user.groups.all()[0].name
	if g == 'Patient':
		appointments = Appointment.objects.filter(patientemail=request.user)
		...
		return render(request,'patientviewappointment.html',d)
	elif g == 'Doctor':
		if request.method == 'POST':
